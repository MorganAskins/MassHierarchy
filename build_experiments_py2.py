...
# This version doesn't use parallel processing in order to be compliant with python2

import math
import matplotlib as lab
...
def main():
    ...
    runs = 10
    backgrounds = smear( build_backgrounds() )
    run_time( backgrounds )
    background_free = [0]*bins

    normal_file = open('normal_events_' + sys.argv[1] + 'ky.txt', 'w')
    inverted_file = open('inverted_events_' + sys.argv[1] + 'ky.txt', 'w')

    
    inf_normal = smear(build_spectrum( 'normal', 'no' ))
    inf_inverted = smear(build_spectrum( 'inverted', 'no' ))
    run_time(inf_normal)
    run_time(inf_inverted)
    ...
    for run in range(runs):
        normal_neutrino = full_build( 'normal', backgrounds )
        inverted_neutrino = full_build( 'inverted', backgrounds )
        write_to_file( normal_file, normal_neutrino )
        write_to_file( inverted_file, inverted_neutrino )
        if run % 10 == 0:
            print ("Event: ", run)
...
def full_build( hierarchy, backgrounds):
    neutrinos = smear( build_spectrum( hierarchy, 'yes' ) )
    run_time(neutrinos)
    ...
    background_subtracted = subtract_spectra( real_spectrum, backgrounds )
...
def combine( spectra ):
    ...
    for i in range(bins):
        for j in range(total_spectra):
            combination[i] += spectra[j][i]
    return combination

def subtract_spectra( positive, negative ):
    difference = [0]*bins
    for i in range(bins):
        difference[i] = positive[i] - negative[i]
    return difference

def run_time( spectrum ):
    for i in range(bins):
        spectrum[i] *= run_length


def smear( spectrum ):
    smeared_spectrum = [0] * bins
    for i in range(bins):
        xvalue = bin_width * i + xmin
        sigma = k*xvalue**0.5
        if ( xvalue - 3*sigma - xmin ) / bin_width > 0:
        ...
        for q in range( bin_min, bin_max ):
            smear_value = q * bin_width + xmin
            smeared_spectrum[q] += spectrum[i] * bin_width * gaus(smear_value, xvalue, sigma)

    return smeared_spectrum
            


def build_spectrum( hierarchy, wiggle ):
    spectrum = []
    inf_inverted = []
    ...
    for i in range(bins):
        xvalue = bin_width * i + xmin
        spectrum.append( cross_section(xvalue) * reactor(xvalue)  * survival(xvalue, hierarchy, wiggle) )
...
def survival( energy, hierarchy, wiggle ):
